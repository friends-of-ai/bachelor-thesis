...
\begin{document}
	...
	\subsection{Classification metrics and confusion matrix}
		...
		\subsubsection{Confusion matrix}
			...
			\en{Finally, the confusion matrix has the following structure, where the number of elements in the class \(P_{i}\) was predicted (P - predicted) although (\(\cong\)) it should have been class \(A_{j}\) (A - actual). All values on the diagonal were correctly predicted (\(\hatset{=}\) when \(i\) and \(j\) are equal).}
