...
\begin{document}
	...
	\section{Optimization process}
		...
		\subsection{Preamble}
			\de{Wie auch in Kapitel ``\hyperref[sec:section_validation_process_preamble]{Preamble}'' bei der Modellvalidierung verwenden wir auch hier als Standard-Setup die gleichen Einstellungen, sofern nicht anders angegeben. Als CNN kommt InceptionV3 zum Einsatz. Das Datenset und auch die Aufteilung in Trainings- und Validierungsdatenset entspricht der Vorgehensweise vom genannten Vorkapitel.}
			\en{As in Chapter ``\hyperref[sec:section_validation_process_preamble]{Preamble}'' for model validation, we use the same settings as the default setup, unless otherwise specified. As CNN we use InceptionV3. The data set and also the division into training and validation data set corresponds to the procedure of the previous chapter.}
