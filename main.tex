...
\begin{document}
	...
	\section{Introduction}
		...
		\label{sec:section_insufficient_amount_of_data}
			...
			\en{Another article only a few years later also addresses this issue. This referred to data that learn from texts and that usually only small or medium sized data sets are available. To improve efficiency also in this case, it is a good idea to improve the algorithms or approach to use data efficiently\autocite{halevy2009unreasonable}:}
	...
	\section{Background}
		...
		\subsection{Image classification}
			...
			\en{Classifications are a process of identifying to which class an unobserved object belongs. Several predefined classes can be specified and, based on their properties, an attempt can be made to classify unknown and previously unobserved objects. The procedure for image classification is similar. The previously mentioned objects are now simply images (figure \ref{fig:cat_or_dog}).}
		...
		\label{sec:section_balanced_training_data_set}
			...
			\begin{figure}[H]
				...
				\caption[Example pictures of a doughnut class]{Example pictures of a doughnut class}
				\label{fig:class_doughnut}
			\end{figure}
	...
	\subsection{Classification metrics and confusion matrix}
		...
		\label{sec:section_machine_learning}
			\de{Maschinelles Lernen ist ein Oberbegriff für die künstliche Generierung von Wissen aus Erfahrung. Es verfolgt den Ansatz des induktiven Lernens (siehe auch Kapitel \nameref{sec:section_inductive_approach}).}
			\en{Machine learning is a generic term for the artificial generation of knowledge from experience. It follows the approach of inductive learning (see also chapter \nameref{sec:section_inductive_approach}).}
			...
			\subsubsection{Overview of current and known convolutional neural networks}
				...
				\en{Last but not least, here are a few current and well-known convolutional neural networks (figure \ref{fig:comparison_cnn}). They differ mainly in the following metrics, whereby in combination each network has its advantages and disadvantages:}
				...
				\begin{itemize}
					...
					\item the model size (for comparison: the model size of \texttt{InceptionV3} is about 180 MB)
				\end{itemize}
...
user$ ml train \
      ...
      --model-file=./data/processed/experiment1/type-of-experiment/model.h5
		...
		\label{sec:section_performance_gpu_cpu}
			...
			\en{Table \ref{tbl:table_performance_comparison} shows a tabular comparison of performance based on the Kaggle flower data set training\autocite{noauthor_flowers_nodate}. This image set consists of 5 classes with about 4242 training images (10 epochs, \texttt{InceptionV3})\autocite{hempel_keras_nodate-1} and was trained with the default values from chapter ``\nameref{sec:section_default_setup}''.}
	...
	\label{sec:section_results}
		...
		\label{sec:section_model_validation}
			...
			\label{sec:section_validation_number_of_train_files}
				...
				\de{Die erste Frage die sich stellt: Welchen Einfluß hat die Anzahl der zu trainierenden Daten auf die Genauigkeit des Modells? Hierzu wird das Standard-Setup aus dem Kapitel ``\nameref{sec:section_default_setup}'' verwendet und das Modell mit einer verschiedenen Anzahl von Trainingsdaten trainiert. Während das Validierungsdatenset immer gleich bleibt, wird die Gesamtanzahl der zu trainierenden Daten von 500 stückweise auf die Gesamtanzahl von 11913 erhöht. Da es sich um einen unausgeglichenen Datensatz handelt, bleibt prozentuall gesehen die Anzahl in den jeweiligen Klassen gleich. Lediglich die Gesamtanzahl wird auf die entsprechenden Werte verändert: 500, 1000, 2000, etc.}
				\en{The first question that comes up: What influence does the amount of data to be trained have on the accuracy of the model? For this purpose, the standard setup from the chapter ``\nameref{sec:section_default_setup}'' is used and the model is trained with a different number of training data. While the validation data set always stays the same, the total number of data to be trained is increased from 500 data sets to the total number of 11913. Since this is an unbalanced data set, the number remains the same in each class in percentage terms. Only the total number is changed to the corresponding values: 500, 1000, 2000, etc. (figure \ref{fig:evaluation_number_train_files}).}
