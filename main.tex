...
\begin{document}
	...
	\section{Background}
		...
		\label{sec:section_further_definitions}
			...
			\label{sec:section_batch_size}
				\de{Die Batch Size definiert die Anzahl der Bilder, die gleichzeitig über das Netzwerk übertragen werden, bevor die Parameter im Netzwerk angepasst werden. Der Vorteil besteht darin, dass man nur die Daten im Speicher halten muss, die für den aktuellen Batch nötig sind. Der Nachteil ist der erhöhte Rechenaufwand und eine ungenaue Schätzung des Gradienten, da sich die Änderung nur auf den aktuellen Batch und nicht auf alle Bilder bezieht\footnote{``What is batch size in neural network?'', itdxer on stackexchange.com, February 9, 2020, \url{https://stats.stackexchange.com/questions/153531/what-is-batch-size-in-neural-network}}.}
	...
	\label{sec:section_validation_process}
		...
		\subsection{Accuracy and evaluations}
			...
			\subsubsection{Influence of number of trained images on accuracy}
				...
				\en{Another interesting point is the fact that increasing the training data theoretically saves computing time up to a certain point. If the data sets are significantly too small, one will never reach an accuracy during training, which models with much more data reach and exceed already with the first epoch. Clearly to see: With 1000 images (orange trace) you get a maximum accuracy of 61.42\% after 18 epochs and about 20 minutes of computing time. If one uses 10 times the amount of images (10000 images - brown trace), one reaches an accuracy of 68.1\% already in the first epoch after six and a half minutes.}
	...
	\section{Optimization process}
		...
		\subsection{Hierarchical classification}
			...
			\de{Um herauszufinden, ob eine Vorklassifizierung das Ergebnis verbessert, werden wir die Klassen in Gruppen aufteilen. Die Idee dabei ist, dass nicht mehr ein Modell alle Klassen vorhersagt, sondern die Bilder erstmal in eine Klasse vorsortiert werden. Die entsprechende Gruppe nimmt dann die eigentliche Klassifizierung vor. Dafür werden wir vorbereitend eine Principal component analysis vornehmen, um die Ähnlichkeit der Klassen aus dem \texttt{food-50} Trainingsdatensatz zu analysieren. Mit dem Ergebnis werden dann Gruppen erstellt. Vorbereitend wird erneut ein Modell für alle Klassen trainiert: InceptionV3, 21 Epochen, Batch Size 16, Lernrate \(\eta = 0.001\) absteigend um Faktor 0.5 alle 7 Epochen. Mittels diesem Modell wird von allen Trainings- und Validierungsdaten eine Vorhersage durchgeführt und der Wahrscheinlichkeitsvektor bestimmt (One Hot Encoding):}
			\en{To find out whether pre-classification improves the result, we will divide the classes into groups. The idea is that no longer one model predicts all classes, but rather the images are pre-sorted into a group. The corresponding group then carries out the actual classification. For this purpose, we will perform a principal component analysis to analyze the similarity of the classes from the \texttt{food-50} training data set. The result will then be used to create groups. In preparation, we will again train a model for all classes: InceptionV3, 21 epochs, batch size 16, learning rate \(\eta = 0.001\) decreasing by factor 0.5 every 7 epochs. Using this model, a prediction is made of all training and validation data and the probability vector is determined (One Hot Encoding):}
				...
				\en{From these probability vectors, an average vector of all \(n\) elements is determined for each class, which really belong to the respective class. Thus, for this 50 class model, we obtain 50 class vectors with the dimension 50:}
				...
				\de{Wie erwartet können wir nun schon sofort ähnliche Klassen erkennen: Getränke (orange), Kuchen (blau) und Salate (grün).}
				\en{As expected, we can now immediately recognize similar classes: drinks (orange), cakes (blue) and salads (green).}
