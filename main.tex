...
\begin{document}
	...
	\section{Background}
		...
		\label{sec:section_image_classification_introduction}
		...
		\label{sec:section_deductive_approach}
			...
			\begin{figure}[H]
				...
				\end{center}
				\caption[Deductive approach]{Deductive approach (the shown model is a white-box model)}
		...
		\label{sec:section_inductive_approach}
			...
			\begin{figure}[H]
				...
				\end{center}
				\caption[Inductive approach]{Inductive approach (the shown model is a black-box model)}
		...
		\label{sec:section_balanced_training_data_set}
			...
			\en{Accessing data is often not that easy. Every data source has its own special features. One way to access data would be an automatic crawling of image databases (Flickr\footnote{\textit{Flickr images} - \textit{Doughnut}. \tiny\textbf{URL}\footnotesize: \url{https://www.flickr.com/search/?text=doughnut}}), search engines (Google\footnote{\textit{Google images} - \textit{Doughnut}. \tiny\textbf{URL}\footnotesize: \url{https://www.google.de/search?q=doughnut&tbm=isch}}, Bing\footnote{\textit{Bing images} - \textit{Doughnut}. \tiny\textbf{URL}\footnotesize: \url{https://www.bing.com/images/search?q=doughnut}}) or reviews (TripAdvisor), in which images appear and can be associated with the text (``This here is the best doughnut I've ever eaten.''). A creative approach is an advantage here.}
	...
	\subsection{Classification metrics and confusion matrix}
		...
		\label{sec:section_loss_function}
			...
			\de{Für die Fehlerrückführung im Backpropagation-Algorithmus wird jedoch nicht die euklidische Norm (\(L_r(\vartheta, \boldsymbol\lambda)\)), sondern der Vektor selbst benötigt (\(\boldsymbol L(\vartheta, \boldsymbol\lambda)\)) und in dieser Arbeit nachfolgend weiter verwendet:}
			\en{For the backpropagation algorithm not the euclidean norm \(L_r(\vartheta, \boldsymbol\lambda)\) is used, but the vector itself is needed and will be further used in this thesis:}

			\begin{equation}
				\label{eq:equation_loss_function}
				\boldsymbol L(\vartheta, \boldsymbol\lambda) := \boldsymbol\lambda - \boldsymbol g(\vartheta)
			\end{equation}
		...
		\subsubsection{Accuracy}
			...
			\en{The accuracy of the entire model is a good indication of the performance of the model. However, a problem occurs in extreme cases where assumptions can no longer be made reliably. For example, if one is working with an unbalanced data set\autocite{geron2017supervisedlearningConfusionMatrix}. Example: Suppose one has a model that always predicts the class \(class_1\). The class \(class_1\) consists of 9,990 elements and from the other classes \(class_2\) to \(class_n\) one has exactly 10. Then the confusion matrix looks like in the following figure (Figure \ref{tbl:figure_confusion_matrix_2}).}
		...
		\label{sec:section_machine_learning}
			...
			\label{sec:section_convolutional_neural_network}
				...
				\en{With a number of \(t\) kernels, \(t\) feature maps are created in which features defined in the filters are highlighted. This process is also called convolution\autocite{wu2017introductionConvolution}.}
				
				\begin{figure}[H]
					\centering
					\scalebox{1.0}{
						\begin{tabular}{ ccccccc }
							(a) input matrix \(I_{xy}\) & & (b) kernel \(k_{nn}\) & & (c) feature map \(I^*_{xy}\) & & (d) pooling \\
							\\
				...
				\en{Neural networks that make use of convolutional layers are called \acfp{cnn} and have made a decisive contribution to the progress of image classification and also in other areas like speech recognition\autocite{geron2017supervisedlearningCNN}. In addition to the convolutional layers, a \ac{cnn} has other special layers that differ from normal neural networks: For example the pooling layer. In a pooling layer, unnecessary information is discarded and feature maps are reduced in size (Figure \ref{fig:simple-convolution-and-simple-pooling}d). This process reduces the number of parameters and the computational
complexity of the model\autocite{o2015introductionPoolingLayer}. The convolutional layer and the pooling layer usually alternate until a large vector is created at the end (instead of matrix \(I_{xy}\)). This vector can be processed by a normal neural network and finally ends with the output of the probability vector \(\boldsymbol\lambda\) as described in chapter ``\hyperref[sec:section_loss_function]{Loss function}'' (Figure \ref{fig:traditional-convolutional-network}).}
			...
			\label{sec:section_transfer_learning}
				\de{\acp{cnn} sind großartig und haben einen entscheidenden Beitrag zur Klassifizierung von Bilder beigetragen. Mit der Gründung der Forschungsdatenbank ImageNet im Jahre 2006 werden jährliche Wettbewerbe veranstaltet, um entwickelte Neuronale Netzwerke miteinander zu vergleichen. ImageNet ist eine Bilderdatenbank mit mehr als 14 Millionen Bildern. Ein \ac{cnn} namens AlexNet im Jahr 2012 einen Top-5 Fehler von 15,3\%. Das klingt nicht viel, jedoch gewann dieses Modell bei der Bildklassifizierungsaufgabe des ImageNet Wettbewerbes ILSVRC 2012\footnote{\textit{ILSVRC 2012}. \tiny\textbf{URL}\footnotesize: \url{http://image-net.org/challenges/LSVRC/2012/}} noch weit vor dem zweiten Platz mit einem Top-5 Fehler von mehr als 26\%\autocite{ning2019prototypingICBoDL}. Das besondere dabei ist die Neuartigkeit der CNN Technik. Die Modellgenauigkeiten können jährlich verbessert werden. Ein CNN \autocite{ResNet50} aus dem Jahre 2015 erreichte eine Modellgenauigkeit von 5,3\%. Aber die Architektur von einem \ac{cnn} hat ein Problem. Alle Convolutional Layer sind vom Beginn an zufällig initialisiert und enthalten noch keine Muster. Damit sie zuverlässig funktioniert, muss sie mit vielen Bildern trainiert werden. Würde man vom Scratch an ein \ac{cnn} selbst entwickeln und verwenden, so müssen alle Convolutional Layer auch vorab trainiert werden.}
				\en{\acp{cnn} have made a significant contribution to the classification of images.\autocite{geron2017supervisedlearningCNN}\textsuperscript{,}\autocite{ning2019prototypingICBoDL} With the foundation of the research database ImageNet in 2006, annual competitions are organized to compare developed neural networks. ImageNet is an image database with more than 14 million images. A CNN called \texttt{AlexNet} in 2012 got a top-5 error of 15.3\%. That doesn't sound a lot, but this model won the ImageNet competition ILSVRC 2012\footnote{\textit{ILSVRC 2012}. \tiny\textbf{URL}\footnotesize: \url{http://image-net.org/challenges/LSVRC/2012/}} far ahead of the second place with a top-5 error rate of more than 26\%. The special thing about this is the innovation of the \ac{cnn} technology. With this technology the model accuracy can be improved yearly. A \ac{cnn} called \texttt{ResNet-50} from the year 2015 achieved a model accuracy of 5.3\%\autocite{alom2018historyHistory}. But the architecture of a \ac{cnn} has a problem. All convolutional layers are randomly initialized from the beginning and do not yet contain any patterns. For it to work reliably, it needs to be trained with many images. If one would develop and use a \ac{cnn} from scratch, all convolutional layers have to be trained in advance.}
	...
	\label{sec:section_considerations_and_implementation}
		...
		\subsection{Research questions and hypothesis section}
...
user$ ml train \
      ...
      --model-file=./data/processed/experiment1/type-of-experiment/model.h5
		...
		\label{sec:section_performance_gpu_cpu}
			...
			\en{If one intends to implement and optimize \acp{dnn}, the calculations must take place on the \ac{gpu}. It is also possible to run calculations on the \ac{cpu}. Also, the installation of the Keras library (see chapter ``\nameref{sec:section_keras_v2_2_4}'') for \ac{cpu} driven computations is much easier, because the installation of the \ac{gpu} drivers is not necessary. The disadvantage of training without GPU support is that it takes longer to train larger models. Sufficiently trained models for the classification of images, for example, are only achieved after several training units. A Training unit requires a lot of computing power in the form of many matrix operations. A \ac{gpu} is predestined for matrix operations\autocite{fatahalian2004understanding}.}
			...
			\en{While the type of computing device (\ac{cpu} or \ac{gpu}) makes no difference in terms of preparation and postprocessing, it makes a significant difference in training time. The slowest \ac{cpu} takes more than 80 times as many times as the fastest graphics card unit. The choice of the computing device for all further tests clearly falls on the \ac{gpu}. All experiments (unless otherwise specified) were trained on a Nvidia GTX 1060 graphic card with 6 Gbyte of memory.}
		...
		\label{sec:section_experiment_setup}
			...
			\label{sec:section_software_specifics}
				...
				\label{sec:section_keras_v2_2_4}
	...
	\label{sec:section_results}
		...
		\en{In this part of the thesis the investigations and the corresponding evaluations will be presented. For each chapter models with the given parameters and techniques have been created with which image classifications can be performed. The models have to be trained, whereby a decision has to be made on many variable parameters like the learning rate \(\eta\), the optimizers, but also things such as the \ac{cnn} model. The basis is a standard setup, which was explained in chapter ``\nameref{sec:section_default_setup}''. Starting from this standard setup, the variable parameters are changed and discussed in the corresponding chapters. The purpose is to find the parameters with which the recognition accuracy of the models is high.}
		...
		\label{sec:section_model_validation}
			...
			\label{sec:section_validation_number_of_train_files}
				...
				\en{The first question that comes up: What influence does the amount of data to be trained have on the accuracy of the model? For this purpose, the standard setup from the chapter ``\nameref{sec:section_default_setup}'' is used and the model is trained with a different number of training data. While the validation data set always stays the same, the total number of data to be trained is increased from 500 data elements to the total number of 11,913. Since this is an unbalanced data set, the number remains the same in each class in percentage terms. Only the total number is changed to the corresponding values: 500, 1,000, 2,000, etc. (Figure \ref{fig:evaluation_number_train_files}).}
				...
				\de{Wie erwartet hat die Anzahl der zu trainierenden Bilder einen entscheidenden Einfluß auf die Modellgenauigkeit. Während mit 500 Bildern eine Genauigkeit von 45,76\% erreicht werden kann, sind es mit fast 12.000 Bildern schon 83,59\% nach 21 Epochen. Mit jeder Erhöhung der Bildanzahl kann man eine Verbesserung der Genauigkeit feststellen. Der Anstieg der Genauigkeit erfolgt nicht linear mit Zunahme der Trainingsdatensätze. Obwohl die Genauigkeit im oberen Bereich (mehr als 8,000 Bilder) immer noch zu steigen scheint, so sind die Sprünge nicht mehr sehr groß und bewegen sich offenbar an eine Grenze. Für weitere Nachforschungen könnte man noch herausfinden, ab welcher Anzahl von Trainingsdatensätzen keine weiteren signifikanten Steigerungen in der Genauigkeiten mehr möglich sind. Da in diesem Datenset keine weiteren Daten mehr zur Verfügung standen, wird dieses Vorhaben hier nicht weiter verfolgt.}
				\en{As expected, the number of images to be trained has a significant influence on the model accuracy. While with 500 images an accuracy of 45.76\% can be achieved, with almost 12,000 images it is already 83.59\% after 21 epochs. With every increase in the number of images, an improvement in accuracy can be observed. The improvement in accuracy is not linear with an increase in training data elements. Although the accuracy in the upper range (more than 5,000 images) still seems to increase, the jumps are not very big anymore and seem to be moving to a limit. For further research one could still find out from which number of training data elements no further significant increase in accuracy is possible. Since no further data was available in this data set, this thesis will not be pursued further here.}
				
				\de{Ein weiterer interessanter Punkt ist die Tatsache, dass mit der Erhöhung der Trainingsdaten theoretisch bis zu einem gewissen Punkt Rechenzeit eingespart werden kann. Das Beispiel mit einem Datenset von 11.913 Bilder (hellblaue Linie) erreicht schon in der ersten Epoche nach sechseinhalb Minuten Trainingszeit eine Genauigkeit von 68,1\%. Bei 1.000 Bildern (orangene Linie) kommt man maximal auf eine Genauigkeit von 61,42\% nach 18 Epochen und ungefähr 20 Minuten Rechenzeit. Ein signifikant zu kleines Datenset erreicht in diesem Fall nicht die Modellgenauigkeit, die ein etwa zehn mal so großes mit der ersten Epoche (und damit weniger Zeit) erreicht.}
				\en{Another interesting point is the fact that increasing the training data can theoretically save computing time. The example with a data set of 11,913 images (light blue trace) reaches an accuracy of 68.1\% after six and a half minutes of training already in the first epoch. With 1,000 images (orange trace), the maximum accuracy is 61.42\% after 18 epochs and about 20 minutes computing time. In this case, a significantly too small data set does not reach the model accuracy that a data set about ten times as large reaches with the first epoch and less time.}
			...
			\label{sec:section_validation_comparison_cnn_models}
				...
				\en{There are currently many \ac{cnn} models and each year with the annual ImageNet competition new models with better accuracy are released. These models are trained with the ImageNet data set and the results are compared using this data set. What is the result with smaller data sets? The training result according to the standard setup and with different \ac{cnn} models of eight models is as follows (Figure \ref{fig:evaluation_different_models}).}
			...
			\label{sec:section_use_of_the_transfer_learning_approach}
				...
				\de{Wie erwartet erziehlt das Modell mit dem Transfer Learning Ansatz ein besseres Ergebnis bei gleicher Rechenzeit (blaue Linie versus orangene Linie). In der ersten Epoche erreicht das Modell ohne Transfer Learning Ansatz einen Genauigkeitswert von 8,5\% und steigert sich in 21 Lernepochen auf 38,2\%. Die Trainingsgenauigkeit mit 58,8\% liegt dabei noch weit unter den Werten, welche mit TL nach wenigen Epochen erreicht werden. Dies bedeutet, das ein Lernen von Features mit den vorhandenen Daten weiter erfolgen kann. Beginnt man mit einem trainierten Netzwerk, so erreicht man schon mit der ersten Epoche einen besseren Genauigkeitswert von 68,9\%. Das ist ein Sprung von 30\% als der derzeit beste Wert ohne TL Ansatz. Die Genauigkeit kann weiter bis 83,6\% in der 21. Epoche gesteigert werden. Was passiert nun, wenn der Datensatz ohne \ac{tl} Ansatz statt der bisher verwendeten 21 Epochen mit 49 Epoche trainiert wird? Es folgen zwei Experimente: Im ersten Experiment wird ein Dropout von \(dropout=0.5\) wie bisher aus dem Standard-Setup verwendet. Im zweitem Experiment wird ein Dropout von \(dropout=0.0\) verwendet um ein Overfitting zu erzwingen und die Konzentration auf das Erlernen der Erkennungsmerkmale zu legen (Abbildung \ref{fig:evaluation_transfer_learning_without}).}
				\en{As expected, the model with the \ac{tl} approach achieves a better result in the same computing time (blue trace versus orange trace). In the first epoch the model without transfer learning approach reaches an accuracy value of 8.5\% and increases to 38.2\% in 21 learning epochs. The training accuracy of 58.8\% is still far below the values achieved with \ac{tl} after a few epochs. This means that learning of features can be continued theoretically with the existing data set (see chapter ``\nameref{sec:section_overfitting}''). If one starts with a pre-trained network, an accuracy value of 68.9\% is achieved in the first epoch.  This is an improvement of 30\% as the current best value without TL approach. The accuracy can be further increased to 83.6\% in the 21st epoch. What happens now, if the data set without \ac{tl} approach is trained with 49 epochs instead of the 21 epochs used so far? Two experiments follow: The first experiment (green trace) uses a dropout of \(dropout=0.5\) as before from the default setup (orange trace). In the second experiment (red trace), a dropout of \(dropout=0.0\) is used to force an overfitting and focus on learning the recognition features (Figure \ref{fig:evaluation_transfer_learning_without}).}
				...
				\de{Mit weiteren 28 Lernepochen konnte die Modellgenauigkeit nochmals bei beiden Experimenten auf 10\% angehoben werden. Dabei ließ sich vor allem mit einem Dropout von \(dropout=0.0\) eine Genauigkeit von 48,2\% erreichen (immer noch unter dem Wert mit TL Ansatz), wobei die Trainingsgenauigkeit schon bei 99,59\% angelangt ist und ein weiteres Generalisieren ist damit theoretisch nicht mehr möglich. Um hier weitere Fortschritte zu erzielen und bessere Genauigkeiten auf bisher unbekannte Bilder zu erreichen, werden weitere Bilder zum Lernen von Featues benötigt. Fazit: Für das Trainieren von Ansätzen ohne TL reicht das aktuell vorhandene Bilderset nicht mehr aus um bessere oder gleiche Werte wie beim TL Ansatz zu erreichen. Der TL Ansatz sollte gerade bei der Verwendung von kleinen Datensätze in Betracht gezogen werden.}
				\en{With further 28 learning epochs, the model accuracy was again increased to 10\% in both experiments. Especially with a dropout of \(dropout=0.0\) an accuracy of 48.2\% could be achieved (still below the value with \ac{tl} approach), whereas the training accuracy has already reached 99.59\% and a further generalization is theoretically no longer possible. In order to make further progress and to achieve better accuracies on previously unseen images, more images are needed to learn features. Conclusion: For the training of approaches without \ac{tl} the currently available image set is no longer sufficient to achieve better or the same values as with \ac{tl} approaches. Especially when using small data sets, the \ac{tl} approach should be considered.}
			...
			\subsubsection{Influence of different error optimizers}
				\de{Der Optimierer im Trainingsprozess dient dazu den Wert der Verlustfunktion zu verringern. Damit wird versucht den Wert der Vorhersagen so korrekt wie möglich wiederzugeben. Der Wert der Verlustfunktion ergibt sich aus der Differenz des erwarteten Wertes und des geschätzten Wertes der Funktion. Der einfachste Ansatz ist das Gradientenabstiegsverfahren\footnote{\textit{Gradient descent}. en. Page Version ID: 943737051. Mar. 2020. \tiny\textbf{URL}\footnotesize: \url{https://en.wikipedia.org/w/index.php?title=Gradient_descent&oldid=943737051} (visited on 03/07/2020)}, welches im Laufe der Zeit immer mehr verbessert wurde und weitere Optimierungsverfahren entstanden. Die Lernrate \(\eta\) bestimmt den Wert der Gewichtung, mit dem der Fehler korrigiert wird (siehe auch Kapitel ``\hyperref[sec:section_loss_function]{Loss function}''). Der neu ermittelte Wahrscheinlichkeitsvektor \(\boldsymbol \lambda\) wird dem Backpropagation Algorithmus zurückgegeben:}
				\en{The optimizer in the training process is used to minimize the value of the loss function. This is an attempt to reflect the value of the predictions as correctly as possible. The value of the loss function is the difference between the expected value and the estimated value of the classification function. The simplest approach is the gradient descent algorithm\footnote{\textit{Gradient descent}. en. Page Version ID: 943737051. Mar. 2020. \tiny\textbf{URL}\footnotesize: \url{https://en.wikipedia.org/w/index.php?title=Gradient_descent&oldid=943737051} (visited on 03/07/2020)}, which has been improved over time and further optimization algorithms have been developed. The learning rate \(\eta\) determines the value of the weighting with which the error is corrected (see also chapter ``\hyperref[sec:section_loss_function]{Loss function}''). The recalculated probability vector \(\boldsymbol \lambda_{new}\) is passed back to the backpropagation algorithm:}

				\begin{equation}
					\boldsymbol \lambda_{new} = \boldsymbol \lambda_{current} - \eta \cdot \boldsymbol L(\vartheta, \boldsymbol \lambda_{current})
				\end{equation}
				...
				\label{sec:section_validation_comparison_optimizer}
					...
					\begin{figure}[H]
						...
						\end{center}
						\vspace*{-20pt}
						\caption{Overview of best optimizer (validation)}
						\label{fig:evaluation_optimizer}
					\end{figure}
	
					\de{Wie man sehen kann, fällt das Erbebnis sehr unterschiedlich aus. Während das Gradientenabstiegsverfahren mit und ohne Nesterov und das Adagrad Verfahren gute Werte liefern (82,0\% bis 83,6\% Genauigkeit) sehen die anderen Verfahren auf den ersten Blick nicht sehr brauchbar aus. Sie konvergieren sehr langsam: Adadelta, Adam und RMSprop.}
					\en{As one can see, the results are very different. While the gradient descent method with and without Nesterov and the Adagrad method provide good values (82.0\% to 83.6\% accuracy), the other methods do not look very useful at first sight. They converge very slowly: Adadelta, Adam and RMSprop (see also training validation in Figure \ref{fig:evaluation_optimizer_train}).}
					
					\begin{figure}[H]
						\begin{center}
							\scalebox{1.0}{
								\inputpgf{images/evaluation}{best_optimizer_train.pgf}
							}
						\end{center}
						\vspace*{-20pt}
						\caption{Overview of best optimizer (training)}
						\label{fig:evaluation_optimizer_train}
					\end{figure}

					\de{Alle Verfahren wurden entsprechend der Keras Dokumentation mit den empfohlenen Einstellungen durchgeführt\footnote{Usage of optimizers, https://keras.io, February 22, 2020, \url{https://keras.io/optimizers/}}. Bei allen Verfahren wurde eine Lernrate von \(\eta = 0.001\) gewählt.}
					\en{All procedures were performed with the recommended settings according to the Keras documentation\footnote{Usage of optimizers, https://keras.io, February 22, 2020, \url{https://keras.io/optimizers/}}. A learning rate of \(\eta = 0.001\) was chosen for all procedures.}
				...
				\label{sec:section_influence_of_the_momentum_and_the_nesterov_momentum}
					\de{Die beiden besten Verfahren aus dem vorhergehendem Kapitel ``\hyperref[sec:section_validation_comparison_optimizer]{Comparison optimizer}'' SGD mit Nesterov und SGD ohne Nesterov sollen hier genauer untersucht werden. Dabei wird diesmal das Augenmerk auf das Momentum gelegt, welches im Standard-Setup mit 0,9 angegeben wird. Anders als bei der einfachen SGD Methode, merkt sich die Momentum-Methode den Wert der letzte Aktualisierung von \(\boldsymbol \lambda\) und bestimmt mit Hilfe des Momentums \(\alpha\) wie hoch der Einfluss der letzten Änderung zusätzlich auf die aktuelle Änderung haben soll. Die Aktualisierung wird in die Richtung "beschleunigt", in welche sich die Fehlerkorrektur offensichtlich aktuell bewegt.}
					\en{The two best methods from the previous chapter ``\hyperref[sec:section_validation_comparison_optimizer]{Comparison optimizer}'' SGD with Nesterov and SGD without Nesterov will be examined here in more detail. This time, the focus is on the momentum, which is specified as 0.9 in the standard setup. In contrast to the simple SGD method, the momentum method remembers the value of the last update of \(\boldsymbol \lambda\) and uses the momentum \(\alpha\) to define how high the influence of the last change of \(\boldsymbol \lambda\) additionally affects the current change of \(\boldsymbol \lambda\) (Equation \ref{eq:equation_momentum_lambda_difference}). The update is "accelerated" in the direction in which the error correction is obviously currently moving (Equation \ref{eq:equation_momentum_lambda}).}

					\begin{equation}
						\label{eq:equation_momentum_lambda_difference}
						\nabla \boldsymbol \lambda_{new} = \alpha \cdot \nabla \boldsymbol \lambda_{old} - \eta \cdot \boldsymbol L(\vartheta, \boldsymbol \lambda_{current})
					\end{equation}

					\begin{equation}
						\label{eq:equation_momentum_lambda}
						\boldsymbol \lambda_{new} = \boldsymbol \lambda_{current} + \nabla \boldsymbol \lambda_{new}
					\end{equation}

					\de{Das Nesterov Momentum ist eine einfache Änderung des normalen Momentums. Die Berechnung der Fehlerfunktion erfolgt nicht mehr nur aus dem aktuellem Wert von \(\boldsymbol \lambda\), sondern bezieht die derzeitige Änderung durch das Momentum mit ein (Gleichung \ref{eq:equation_momentum_lambda_nesterov}). Dies soll helfen Richtungsänderungen schneller zu korrigieren oder Overshoots zu vermeiden.}
					\en{The Nesterov momentum is a simple modification of the normal momentum. The calculation of the error function is no longer based only on the current value of \(\boldsymbol \lambda\), but also includes the current change by the momentum (equation \ref{eq:equation_momentum_lambda_nesterov}). This should help to correct directional changes faster and avoid overshoots.}

					\begin{equation}
						\label{eq:equation_momentum_lambda_nesterov}
						\boldsymbol \lambda_{new} = \boldsymbol \lambda_{current} + \alpha \cdot \nabla \boldsymbol \lambda_{old} - \eta \cdot \boldsymbol L(\vartheta, \boldsymbol \lambda_{current} + \alpha \cdot \nabla \boldsymbol \lambda_{old})
					\end{equation}

					\de{Das Momentum wird in den nächsten beiden Experimenten von 0,5 bis in den Bereich von 0,98 variiert. Die Lernrate startet mit 0,001 und verringert sich nach jeweils 7 Epochen um 50\%. Das erste Experiment startet ohne das Nesterov Momentum (Abbildung \ref{fig:evaluation_momentum_without}), das zweite Experiment findet mit der Nesterov Momentum Methode statt (Abbildung \ref{fig:evaluation_momentum}). Das Ergebnis soll daraufhin verglichen und diskutiert werden.}
					\en{In the next two experiment, the momentum is varied from 0.5 to the range of 0.98 (Figure \ref{fig:evaluation_momentum}). The learning rate starts at 0.001 and decreases by 50\% after every seven epochs. The first experiment starts without the Nesterov momentum (Figure \ref{fig:evaluation_momentum_without}), the second experiment uses the Nesterov momentum method (Figure \ref{fig:evaluation_momentum}). The results will then be compared and discussed.}
					...
					\begin{figure}[H]
						...
						\end{center}
						\vspace*{-20pt}
						\caption[Overview of experiments of different momentum values without Nesterov]{Overview of experiments of different momentum values without Nesterov (\(momentum\ \hat{=}\ \alpha \))}
			...
			\label{sec:section_validation_comparison_different_bs}
				\de{Die Batch Size gehört mit zu den Regularisierungsparameter, da sie einem Overfitting entgegenwirken kann. Wie im Kapitel \hyperref[sec:section_batch_size]{``Batch Size''} beschrieben sollte der Lernprozess in mehrere kleinere Mini-Batches unterteilen werden. Die Frage in dieser Auswertung ist, wie sich unterschiedliche Größen der Batch Size auf die Genauigkeit des Modells auswirken. In einem Paper von Pavlo M. Radiuk aus dem Jahre 2017 brachte die höchstmögliche Batch Size von 1024 die beste Genauigkeit und half einen hohen Grad an Varianz zu vermeiden. Jedoch stellte Pavlo M. Radiuk auch fest, dass die Verbesserungen in den Genauigkeiten in hohen Bereichen nur noch sehr klein waren\autocite{radiuk2017impact}. Auch sei zu erwähnen, dass sein Datensatz aus 60.000 Bildern verteilt in 10 Klassen bestand (gegenüber 50 Klassen mit rund 250 Bildern pro Klasse in dieser Arbeit) und er sehr kleine Bilder verwendete (32x32 Pixel), um so einen hohen Wert der Batch Size zu ermöglichen. Die unterschiedlichen Batch Size Größen werden auch hier auf den \texttt{food-50} Datensatz angewendet (Abbildung \ref{fig:evaluation_overview_influence_batch_size}). Eine höhere Batch Size als 32 konnte aufgrund technischer Gegebenheiten nicht verwendet werden.}
				\en{The batch size is one of the regularization parameters, since it can counteract overfitting. As described in the chapter \hyperref[sec:section_batch_size]{``Batch Size''} the learning process should be divided into several smaller mini-batches. The question in this evaluation is how different sizes of batch size affect the accuracy of the model. In a paper by Pavlo M. Radiuk from 2017, the highest possible batch size of 1,024 provided the best accuracy and helped to avoid a high degree of variance. However, Pavlo M. Radiuk also found that the improvements in accuracy in high ranges were only very small\autocite{radiuk2017impact}. It should also be mentioned that his data set consisted of 60,000 images distributed in 10 classes (compared to 50 classes with about 250 images per class in this thesis) and he used very small images (32x32 pixels) to allow a high value of batch size. The different batch size sizes are also applied to the \texttt{food-50} data set (Figure \ref{fig:evaluation_overview_influence_batch_size}). A batch size higher than 32 could not be used due to technical reasons.}
				...
				\de{Ein höherer Wert der Batch Size führt offensichtlich zu einem schnellerem und vorhersehbarerem (gleichmäßigerem) Anstieg der Validierungsgenauigkeit in den ersten Epochen (hellblaue Linie mit \(batch\_size=32\)), gegenüber einer Batch Size von zum Beispiel 16, bei der die Genauigkeit von Epoche zwei bis sieben ``springt''. Jedoch liefert eine höhere Batch Size nicht das beste Ergebnis am Ende der 21 Epochen. Während ein Wert von 16 bei der Batch Size eine Genauigkeit von 83.52\% erreicht, so erreicht diese nur 82.3\% bei einem Wert von 32. Bis zur Batch Size von 16 kann jedoch gesagt werden, dass mit dem Ansteigen der Größe der Batch-Size immer ein besseres Ergebnis erreicht wird. Ein Erhöhen der Batch-Size verringert die benötigte Rechenzeit wie erwartet. In diesem Fall konnte diese von acht Stunden auf zwei Stunden verbessert werden. Ein Overfitting wird bei einer Batch Size von 2 nur sehr langsam erreicht (Figure \ref{fig:evaluation_overview_influence_batch_size_train}), jedoch ist dieses Modell mit einer Erkennungsgenauigkeit von 67,13\% auch das Schlechteste. Sehr schön zu erkennen ist auch die wesentlich schneller ansteigende Trainingsgenauigkeit bis zum Wert 100\% mit steigender Batch Size. Die Batch Size ist in diesem Fall ebenfalls ein Hyperparameter, welcher optimiert und festgelegt werden muss. Untersuchungen mit einem etwas mehr ausbalanciertem Datenset und einer noch höheren Batch Size sollten angestrebt werden, um den Einfluss der Batch Size auf verschiedene Datensätze weiter zu untersuchen.}
				\en{A higher batch size value obviously leads to a faster and more predictable (more even) increase in validation accuracy in the first epochs (light blue trace with \(batch\_size=32\)), compared to a batch size of 16 (red trace), for example, where the accuracy ``jumps'' from epoch two to seven. However, a higher batch size does not give the best result at the end of the 21 epochs. While a value of 16 for the batch size achieves an accuracy of 83.52\%, it only achieves 82.54\% with a value of 32. Up to a batch size of 16, however, it can be said that as the batch size increases, a better result is always achieved. Increasing the batch size reduces the required computing time as expected. In this case, it has been improved from eight hours to two hours. Overfitting is achieved very slowly with a batch size of 2, but this model is also the worst one with a detection accuracy of 67.13\%. The much faster increasing training accuracy up to 100\% with increasing batch size can be seen very clearly in Figure \ref{fig:evaluation_overview_influence_batch_size_train}. The batch size is in this case also a \ac{hp}, which must be defined before the training and determined experimentally for optimal values. Investigations with a slightly more balanced data set and an even higher batch size should be aimed at to further investigate the influence of batch size on different data sets.}
		...
		\label{sec:section_optimization_process}
			...
			\subsubsection{Comparison of different neural network types}
				...
				\de{In einem Artikel mit dem Titel ``Impact of Fully Connected Layers on Performance of
Convolutional Neural Networks for Image Classification\autocite{basha2020impact}'' kam man zu dem Ergebnis, dass insbesondere tiefe CNNs weniger tiefe FC Layer am Ende benötigen um gute Ergebnisse zu erzielen. Das konnte mit diesem Experiment bestätigt werden. Das beste Ergebnis von 83,52\% liefert der Aufbau ohne Dropout Schicht und nur einem DL. Die Modellgenauigkeiten unterscheiden sich ansonsten in diesem Experiment nicht allzu stark. Überraschenderweise erreicht der komplexeste Aufbau das zweitbesteste Ergebnis von 83,43\%. Es besteht aus einem Dropout-Layer direkt nach der CNN Ausgabe und wird danach um drei weitere DL erweitert. Die jeweils letzte DL Schicht gibt den Wahrscheinlichkeitsvektor \(\boldsymbol\lambda\) zurück. Ein zusätzliches Hinzufügen von weiteren DLs und sogar einer Dropout Schicht ist bei aktuellen CNNs wie \texttt{InceptionV3} nicht notwendig.}
				\en{In an article entitled ``Impact of Fully Connected Layers on Performance of
Convolutional Neural Networks for Image Classification\autocite{basha2020impact}'' one came to the conclusion that especially deep \acp{cnn} need less deep \acp{fcl} at the end to achieve good results. This could be confirmed with this experiment.  The best result of 83.52\% is achieved by the setup without dropout layer and only one \ac{dl}. Otherwise the model accuracies do not differ too much in this experiment. Surprisingly, the most complex setup achieved the second best result of 83.43\%.  It consists of a dropout layer directly after the \ac{cnn} output and is extended by three more \ac{dl} afterwards.  The last \ac{dl} layer returns the probability vector \(\boldsymbol\lambda\). Adding more \acp{dl} and even a dropout layer is not necessary for current \acp{cnn} like \texttt{InceptionV3} for image classification on small data sets.}
			...
			\label{sec:section_data_augmention}
				...
				\de{Die Modellgenauigkeit erhöht sich von 83,6\% auf 85,6\% und steigert sich damit um insgesamt 2\%. Während die Berechnung des Datensatzes ohne Data Augmentation nur zweieinhalb Stunden braucht, benötigt man für den neuen Datensatz die vierfache Zeit. Fazit: Data Augmentation geht mit einer verbesserten Genauigkeit einher, benötigt aber auch mehr Rechenzeit. Wenn Rechenzeit keine Rolle spielt, so ist dies eine Möglichkeit die Genauigkeit etwas zu verbessern. Es sollte erwähnt werden, dass der Datensatz mit der Vergrößerung des Datensatzes ausbalanciert wurde. Das bedeutet, dass sich in jeder Klasse gleich viele Datensätze zum Trainieren befanden. Zu diesem Zeitpunkt kann nicht genau gesagt werden, ob sich die Verbesserung durch die Erhöhung der zur Verfügung stehenden Daten erreicht werden konnte, sondern das Ausbalancieren des Datensatzes ebenfalls zur Verbesserung beitrug. Dies sollte in weiteren Versuchen genauer untersucht werden.}
				\en{The model accuracy increases from 83.6\% to 85.6\% and shows an overall increase of 2\%. While the calculation of the data set without data augmentation takes only two and a half hours, the new data set takes four times as long. Conclusion: Data Augmentation is associated with improved accuracy, but also requires more computing time. If computing time is not important, this is one way to improve the accuracy a little. It should be mentioned that the data set was balanced with data augmentation. That means that there were the same number of elements for training in each class. At this point, it is not clear to say whether this improvement could only be achieved by increasing the available data or whether balancing the data set did also contribute the improvement. This should be investigated in further experiments.}
			...
			\label{sec:section_validation_hierarchical classification}
				...
				\en{In order to find out whether a pre-classification improves the result, the classes are divided into groups. The idea is that no longer one model predicts all classes, but rather the images are pre-sorted into a group. The corresponding group then carries out the actual classification. For this purpose, a principal component analysis is performed to analyze the similarity of the classes from the \texttt{food-50} training data set. The result will then be used to create groups. In preparation, a model will again be trained for all classes: \texttt{InceptionV3}, 21 epochs, batch size 16, learning rate \(\eta = 0.001\) decreasing by factor 0.5 every seven epochs. Using this model, a prediction is made of all training and validation data and the probability vectors are determined:}
					...
					\label{sec:section_k_means_clustering}
						\de{Die 50 Vektoren \(\overline{\boldsymbol\lambda}_{class_{m}}\) der Gleichung \eqref{eq:equation_hierarchical_classification_mean} werden nun mittels \(k\)-means clustering untersucht. Ziel von \(k\)-means ist es, den Datensatz so in \(k\) Partitionen zu teilen, dass die Summe der quadrierten Abweichungen von den Cluster-Schwerpunkten minimal ist. Dabei werden acht Gruppen geclustert (\(k=8\)). Die Verteilung ist wie nachfolgend dargestellt:}
						\en{All 50 vectors \(\overline{\boldsymbol\lambda}\) of the equation \eqref{eq:equation_hierarchical_classification_mean} are now examined by \(k\)-means clustering. The purpose of \(k\)-means is to divide the data set into \(k\) partitions in such a way that the sum of the squared deviations from the cluster centers is minimal\autocite{noauthor_k-means_2020}. Eight groups are now clustered (\(k=8\)). The distribution is shown in Table \ref{tbl:table_grouped_classes_short_view}. A more detailed version with all class names can be found in Table \ref{fig:analysis_grouped_classes_with_k_means} in the appendix. There is also a visual representation (Figure \ref{tbl:table_grouped_classes}).}
						...
						\begin{table}[htb]
							...
							\captionof{table}[Grouped classes with \(k\)-means]{Grouped classes with \(k\)-means. \textsuperscript{*}=All accuracies \(acc\) are top-1 accuracies.}\label{tbl:table_grouped_classes_short_view}
						\end{table}


						\de{Die Klassen aus dem Datensatz \texttt{food-50} wurden ungleichmäßig in den Gruppen verteilt. Die Gruppe \texttt{group\textsubscript{0}} enthält insgesamt 18 Klassen. Das entspricht 31,1\% des gesamten Datensatzes. Die Gruppe \texttt{group\textsubscript{4}} hingegen enthält nur eine Klasse mit 63 Dateien, was 2,1\% des gesamten Datensatzes entspricht. Mit dieser Verteilung werden nun neun Trainings- und Validierungsdatensätze erstellt. Der erste Datensatz enthält die Trainingsdaten für das Vorklassifizierungsmodel. Dieses besteht aus acht neuen Klassen (\texttt{group\textsubscript{0}} bis \texttt{group\textsubscript{7}}). In jede dieser neuen Klassen befinden sich alle Elemente der ihr zugeordneten Gruppen (\texttt{baked\_beans}, etc.). Die anderen acht Datensätze werden für Modelle trainiert, welche die Unterklassen klassifizieren sollen. In diese Datensätze werden die Klassen aus dem Datensatz \texttt{food-50} entsprechend verteilt. Die neue Klasse \texttt{group\textsubscript{4}} enthält somit nur eine Klasse mit dem Namen \texttt{martini}. Die neun Datensätze werden nach dem Standard-Setup trainiert, womit die top-1 Validierungsgenauigkeit \(acc^*\) jeweils ermittelt werden kann. Die Formel \eqref{eqn:equation_statistic_top_1} errechnet mit den ermittelten Genauigkeiten und der Verteilung der Dateien eine Gesamtgenauigkeit. Es wurde eine Gesamtgenauigkeit von 79,23\% für dieses Model errechnet.}
						\en{The classes from the data set \texttt{food-50} were distributed in unbalanced groups. The group \texttt{group\textsubscript{0}} contains 18 classes in total. This corresponds to 31.1\% of the entire data set. The group \texttt{group\textsubscript{4}} contains only one class with 63 files, which is 2.1\% of the entire data set. With this distribution, nine training and validation data sets are now created. The first data set contains the training and validation data for the pre-classification model. This consists of eight new classes (\texttt{group\textsubscript{0}} to \texttt{group\textsubscript{7}}). Each of these new classes contains all elements of the groups assigned to it (\texttt{baked\_beans}, etc.). The other eight data sets are trained for models that will classify the subclasses. In each of these data sets, the classes from the data set \texttt{food-50} are distributed accordingly. For example, the new class \texttt{group\textsubscript{4}} contains only one class named \texttt{martini}.  The nine data sets are trained according to the default setup, which allows to determine the top-1 validation accuracy \(acc^*\) in each case. The formula \eqref{eqn:equation_statistic_top_1} calculates an overall accuracy using the determined accuracies and the distribution of the files. An overall accuracy of 79.23\% was calculated for this model.}

					\paragraph{Agglomerative hierarchical clustering}
						\de{Die agglomerativen Clusterverfahren beschreiben ein Klassifizierungsverfahren, bei denen zunächst jedes Objekt einen Cluster bildet und dann schrittweise die bereits gebildeten Cluster zu immer größeren zusammengefasst werden, bis die Anzahl gewünschter Gruppen erreicht ist. Eine genaue Erklärung zum Vorgehen und Algorithmus kann unter Wikipedia\autocite{noauthor_hierarchical_2020} eingesehen werden. Es wird hier nicht näher darauf eingegangen. Es wurde die Funktion \texttt{AgglomerativeClustering}\footnote{\textit{sklearn.cluster.AgglomerativeClustering}. en. Page Mar. 2020. \tiny\textbf{URL}\footnotesize: \url{https://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html} (visited on 03/08/2020)} aus der sklearn Bibliothek von Python verwendet. Dabei wurden die Parameter \(n_clusters=8\), \(affinity='euclidean'\) und \(linkage='ward'\) übergeben.}
						\en{The agglomerative hierarchical clustering algorithm describes a classification procedure in which each object first forms a cluster.  Clusters that have already been formed will then be combined to form larger and larger clusters, until the number of desired groups is reached. A detailed explanation of the procedure and algorithm can be found at Wikipedia\autocite{noauthor_hierarchical_2020}. It will not be discussed here in detail. The function \texttt{AgglomerativeClustering}\footnote{\textit{sklearn.cluster.AgglomerativeClustering}. en. Page Mar. 2020. \tiny\textbf{URL}\footnotesize: \url{https://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html} (visited on 03/08/2020)} from the sklearn library from Python was used. The parameters \texttt{n\_clusters=8}, \texttt{affinity=`euclidean'} and \texttt{linkage=`ward'} were chosen.}

						\de{Der Aufbau und die Vorgehensweise entspricht weitestgehend der aus dem Kapitel ``\nameref{sec:section_k_means_clustering}''. Einzig das Clustering wird gegen das agglomerative hierarchical clustering ausgetauscht, womit eine andere Verteilung der Gruppen entsteht. Die Verteilung ist in Tabelle \ref{tbl:table_grouped_classes_ahc_short_view} einzusehen. Eine detailiertere Variante inkl. Klassennamen befindet sich im Anhang (Tabelle \ref{tbl:table_grouped_classes_ahc}). Ebenso befindet sich dort eine visuelle Darstellung der Klassen im Raum (Abbildung \ref{fig:analysis_grouped_classes_with_ahc}). Die Verteilung ist diesmal sehr ähnlich, jedoch noch etwas mehr ausbalancierter. Die Gruppe \texttt{group\textsubscript{0}} enthält diesmal 22 Grupppen (42,91\% der Gesamtverteilung). Gruppe mit nur einer Gruppe sind auch diesmal vorhanden. Die errechnete Gesamtgenauigkeit beträgt diesmal 78,60\%, was ein noch schlechterer Wert als im vorhigen Kapitel ist.}
						\en{The structure and the procedure for determining the model accuracy corresponds almost to the procedure mentioned in the chapter ``\nameref{sec:section_k_means_clustering}''. Only the clustering method is replaced by the agglomerative hierarchical clustering method, which results in a different distribution of groups. The distribution can be seen in table \ref{tbl:table_grouped_classes_ahc_short_view}. A more detailed variant including class names can be found in the appendix (table \ref{tbl:table_grouped_classes_ahc}). There is also a visual representation of the classes (Figure \ref{fig:analysis_grouped_classes_with_ahc}). The distribution is very similar this time, but a bit more unbalanced. The group \texttt{group\textsubscript{0}} contains 22 groups this time (42.91\% of the total distribution). Groups with only one group are also present. The calculated overall accuracy is 78.60\%, which is lesser than in the previous chapter.}
						...
						\begin{table}[htb]
							...
							\captionof{table}[Grouped classes with agglomerative hierarchical clustering]{Grouped classes with agglomerative hierarchical clustering. \textsuperscript{*}=All accuracies \(acc\) are top-1 accuracies.}\label{tbl:table_grouped_classes_ahc_short_view}
						\end{table}

				\paragraph{Conclusion of the hierarchical classification}
						\de{Wie man sehen kann ist der Aufwand zum Erstellen beider Umgebung sehr hoch. Auch die Rechenzeit erhöht sich in diesem Fall auf etwas das Doppelte (die Gruppen \texttt{group\textsubscript{0}} bis \texttt{group\textsubscript{7}} enthalten zusammen gesehen die gleichen Daten wie der Vorklassifizierer). Auch wenn der Ansatz vielversprechend klang, so ist er es nicht. Die bestimmte Genauigkeit liegt unter dem Durchschnitt. Hierarchische Klassifizierung für wenige Gruppen durchzuführen, führt zu keiner Verbesserung. Im Gegenteil, beide verschlechterten sich sogar. An dieser Stelle sei nochmal erwähnt, dass die Klassen unausbalanciert waren. Man könnte zukünftig einen Clusteralgorithmus mit gleicher Verteilung probieren, um zu prüfen, ob Clustering mit gleichverteilten Gruppen zu einer Verbesserung führt.}
						\en{As one can see, the complexity to create both of the above mentioned environments is very high. Also the computing time is doubled in this case (the groups \texttt{group\textsubscript{0}} to \texttt{group\textsubscript{7}} together contain the same data elements as the pre-classifier). Although the approach seemed to be promising, it is not. The specific accuracy is below average. Performing hierarchical classification for a few groups does not lead to an improvement. On the contrary, both accuracy has decreased. At this point it should be mentioned again that the groups were unbalanced. In the future, one could try a clustering algorithm with equal distribution to see if clustering with equally distributed groups leads to an improvement.}
			...
			\subsubsection{Binary classifiers}
				...
				\en{A binary classifier knows only two states.  Either a property is true or it is not true. For example, if one uses a classifier of the class \texttt{salad} to classify an element, this classifier returns the probability that this image is salad or not (Equation \eqref{eq:equation_binary_probability_vector}).}
				...
				\de{Für die binäre Klassifikation mit mehr als zwei Klassen benötigt man für jede Klasse einen eigenen Klassifizierer. Für den hier in der Arbeit verwendeten Essensdatensatz \texttt{food-50} werden 50 Klassifizierungsmodelle erstellt, welche jedes einzelne in der Lage ist eine Aussage zu treffen, mit welcher Wahrscheinlichkeit ein zu klassifizierendes Element zu seiner Klasse gehört oder nicht. Obwohl diese Aussage grundsätzlich der Aussage eines einzelnen bisher verwendeten 50 Klassen Klassifizierers entspricht, so werden hier die Erkennungen auf einzelne Modelle verteilt. Theoretisch sollte sich damit die Vapnik–Chervonenkis Dimension erhöhen. Die Frage die sich nun stellt: Ist es damit möglich die Modellgenauigkeit zu erhöhen?}
				\en{For a binary classification with more than two classes one needs a separate classifier for each class. For the used data set \texttt{food-50} in this thesis 50 classification models are created. Each of them is able to make a statement about the probability that an item to be classified belongs to its class or not.  Although this statement basically corresponds to the statement of a single 50 class classifier used so far, the recognitions are distributed to individual models here. In theory, this should increase the \ac{vc} dimension. The question that now arises: Is it possible to increase the top-1 model total accuracy by distributing the classification over 50 classifiers?}

				\de{Um diese Frage beantworten zu können, wird das bestehende Datenset aus 50 Klassen modifiziert. Für jede Klasse wird ein eigenes Datenset erstellt, welches aus der Klasse ``True'' und der Klasse ``False'' besteht. In die Klasse ``True'' kommen alle Elemente der aktuell betrachteten Klasse, in die Klasse ``False'' alle anderen Elemente aus allen anderen Klassen. Man erhält damit 50 Trainings- und Validierungsdatensätze, welche entsprechend nach dem Standard-Setup einzeln trainiert werden. Am Ende erhält man jeweils 50 Klassifikationsmodelle. Eine Übersicht über das Ergebnis der einzelnen Modellgenauigkeiten kann der Tabelle \ref{tbl:table_binary_classification_overview} im Anhang entnommen werden. Die einzelnen Modellgenauigkeiten sehen auf den ersten Blick vielversprechend aus. Die schlechteste Modellgenauigkeit erreichte die Klasse \texttt{frittata} mit einer Genauigkeit von 98,47\%. Die beste Modellgenauigkeit von 100,00\% wurde für die Klasse \texttt{french\_fries} erreicht.}
				\en{To answer this question, the existing data set of 50 classes will be modified. For each class a separate data set is created, which consists of the class ``True'' and the class ``False''. The class ``True'' contains all elements of the currently considered class, the class ``False'' contains all other elements from all other classes. The result is a set of 50 training and validation data sets (\texttt{food-50-1} to \texttt{food-50-50}), which are trained individually according to the standard setup. At the end you get 50 classification models for each class. An overview of the result of the individual model accuracies can be taken from the table \ref{tbl:table_binary_classification_overview} in the appendix. The individual model accuracies appear to look promising at first view. The worst model accuracy was achieved by the class \texttt{frittata} with an accuracy of 98.47\%. The best model accuracy of 100.00\% was achieved by the class \texttt{french\_fries}.}

				\de{Die 50 erstellten Klassifikatoren werden nun auf die Trainingsdaten, als auch auf die Validierungsdaten angewendet um Lambda zu erhalten. Von den erhaltenden Wahrscheinlichkeitsvektoren wird der True verwendet, um damit den eigentlichen Wahrscheinlichkeitsvektor zu bestimmen (Gleichung \eqref{eq:equation_binary_probability_vector_50_norm}).}
				\en{The 50 created classifiers are now applied to the training data as well as to the validation data to obtain \(\hat{\boldsymbol\lambda}_{class_{m}}\). From the obtained probability vectors \(\hat{\boldsymbol\lambda}_{class_{m}}\) the ``True'' value is used to determine the actual vector (Equation \eqref{eq:equation_binary_probability_vector_50} and \eqref{eq:equation_binary_probability_vector_50_norm}).}

				\begin{equation}
					\label{eq:equation_binary_probability_vector_50}
					\boldsymbol\lambda_{class_{m}}^{*} =
					\left(
						\begin{array}{c}
							p_{True,class_{1}}\\
				...
				\begin{equation}
					\label{eq:equation_binary_probability_vector_50_norm}
					\boldsymbol\lambda_{class_{m}} = 
					\frac{\boldsymbol\lambda_{class_{m}}^{*}}{\| \boldsymbol\lambda_{class_{m}}^{*} \|}
				\end{equation}

				\de{Für den Datensatz \texttt{food-50} erhält man für den Trainingsteil 11.913 Wahrscheinlichkeitsvektoren \(\boldsymbol\lambda\) und für den Validierungsteil 2.953 Wahrscheinlichkeitsvektoren \(\boldsymbol\lambda\) mit der Dimension 50. Mittels dem \(k\)-nearest neighbors Klassifizierungsalgorithmus\autocite{noauthor_k-nearest_2020} wird nun die Modellgenauigkeit bestimmt. Dabei wird eine Klassenzuordnung von \(\boldsymbol\lambda^{val}\) unter Berücksichtigung seiner nächsten \(k\) Nachbarn von (\boldsymbol\lambda^{train}\) vorgenommen. Die dabei entstehenden Modellgenauigkeiten können der nachfolgenden Tabelle entnommen werden.}
				\en{For the data set \texttt{food-50} one obtains for the training part 11,913 vectors \(\boldsymbol\lambda^{train}\) and for the validation part 2,953 vectors \(\boldsymbol\lambda^{val}\) with the dimension 50. Using the \(k\)-nearest neighbors classification algorithm\autocite{noauthor_k-nearest_2020} the model accuracy is now determined. A class assignment of \(\boldsymbol\lambda^{val}\) is determined considering its next \(k\) neighbours of \(\boldsymbol\lambda^{train}\). The resulting model accuracies at different \(k\) values are shown in the following table (Table \ref{tbl:table_binary_classification_acc}).} % sec:section_supervised_learning
			...
			\begin{table}[htb]
				...
				{\def\arraystretch{1}\tabcolsep=5pt
					{\setlength{\extrarowheight}{2pt}
						\begin{tabularx}{\linewidth}{ r  r | r  r | r  r | r  r | r  r }
							\thead[l]{\textit{k}} & \thead[l]{top-1 acc.} & \thead[l]{\textit{k}} & \thead[l]{top-1 acc.} & \thead[l]{\textit{k}} & \thead[l]{top-1 acc.} & \thead[l]{\textit{k}} & \thead[l]{top-1 acc.} & \thead[l]{\textit{k}} & \thead[l]{top-1 acc.} \\
	\hline
						1 & 73.42\% &
						2 & 72.98\% &
						3 & 74.84\% &
				...
				\captionof{table}{Comparison of the model accuracy at different k values.}\label{tbl:table_binary_classification_acc}
			\end{table}

			\de{Das beste Ergebnis mit einer Modellgenauigkeit von 75,75\% konnte mit \(k=9\) erreicht werden. Darüber bzw. darunter fallen die Werte ab. Der Aufwand für den Modellaufbau um binäre Klassifikatoren zu erstellen, ist verglichen mit anderen Verfahren ist sehr hoch. Es benötigt 50 mal mehr Rechenzeit für die Erstellung der Modelle, wohingegen die Modellgenauigkeit, die damit erreicht werden kann, hingegen vergleichsweise gering ist. Beim Datensatz \texttt{food-50} brachte dieser Versuchsaufbau keine Verbesserungen. Das Ergebnis befindet sich im Gegenteil sogar unter den Erwartungen. Binäre Klassifkationen eigenen sich nicht dazu die Modellgenauigkeit von kleinen Datensets zu verbessern.}
			\en{The best result with a model accuracy of 75.75\% was achieved with \(k=9\). Above or below this the accuracy decreases. The complexity of building models to create binary classifiers is very extensive and expensive compared to other methods. It requires 50 times more computing time to create the models compared to a single model, whereas the model accuracy that can be achieved is relatively low. For the data set \texttt{food-50} this experimental setup did not show any improvements. On the contrary, the result is actually below the expectations. Binary classifications are not well suited to improve the model accuracy of small data sets.}
	...
	\section{Summary and outlook}
		\de{Während dieser Arbeit wurden verschiedene Hyperparameter und Techniken zum Trainieren und Validieren von Bildklassifikatoren betrachtet und miteinander verglichen. Das Training fand mit den Standard-Einstellungen aus dem Kapitel ``\nameref{sec:section_default_setup}'' statt.}
		\en{During this thesis different hyperparameters and techniques for training and validating of image classifiers were considered and compared. The training was performed with the default settings from the chapter ``\nameref{sec:section_default_setup}''.}

		\de{Es konnte gezeigt werden, dass mit einem unbalanciertem, händisch gelabelten Datensatz bestehen aus 11.913 Trainingsdaten und 2.953 Validierungsdaten eine Modellgenauigkeit bis zu 85,60\% erreicht wurde. Dabei wurden bis zu 10 Stunden Rechenzeit auf einer NVIDIA GeForce GTX 1060 mit 6GB Speicher benötigt. Bei einem durchschnittlichen Rechenaufwand von zweieinhalb Stunden konnten Modellgenauigkeite bis zu 83,59\% erreicht werden.}
		\en{It could be demonstrated that a model accuracy up to 85.60\% was achieved with an unbalanced, manually labelled data set consisting of 11,913 training elements and 2,953 validation elements. This required up to 10 hours of computing time on a NVIDIA GeForce GTX 1060 with 6 GByte of memory. With an average of two and a half hours of computing time for 21 training epochs, model accuracies of up to 83.59\% were achieved.}

		\de{Die Wahl für die meisten Experimente viel auf das CNN Model \texttt{InceptionV3} (Modellgenauigkeit 83,59\%), obwohl das CNN Modell \texttt{Densenet201} mit 84,58\% genau 1\% weiter vorn liegt. Mit \texttt{InceptionV3} war es jedoch möglich mit weniger Speicherverbrauch in durchschnittlicher Zeit gute Ergebnisse zu erziehlen, während \texttt{Densenet201} deutlich langsamer arbeitet. Für zukunftige weitere Untersuchungen und mit deutlich mehr GPU Performance könnte man die hier genannten Experimente mit deutlich größeren CNN Modellen erneut durchführen.\autocite{upadhyaya2013parallel}\textsuperscript{,}\autocite{cavuoti2013genetic}}
		\en{The choice for most experiments is based on the \ac{cnn} model \texttt{InceptionV3} (model accuracy 83.59\%), although the \ac{cnn} model \texttt{Densenet201} with 84.58\% model accuracy achieves a 1\% higher value with the standard setup (see chapter ``\nameref{sec:section_validation_comparison_cnn_models}''). However, with \texttt{InceptionV3} it was possible to achieve good results with less memory consumption in average time, while \texttt{Densenet201} works significantly slower. For future investigations and with much more \ac{gpu} performance the experiments mentioned here could be repeated with much larger \ac{cnn} models like \texttt{Densenet201}. As an idea the \ac{gpu} parallel computing technology should be mentioned here.\autocite{upadhyaya2013parallel}\textsuperscript{,}\autocite{cavuoti2013genetic}}

		\de{Weiterhin konnte gezeigt werden, das mit steigender Datenmenge die Modellgenauigkeiten permanent verbessert werden konnten (siehe Kapitel ``\nameref{sec:section_validation_number_of_train_files}''). Es scheint so, dass das Maximum an Modellgenauigkeit mit 11,913 Trainingsbildern noch nicht erreicht wurde (siehe auch Kapitel Data Aumentation). Es sollte hier weiter untersucht werden, inwiefern eine weiter steigende Datenmenge die Modellgenauigkeit weiter verbessert und ab wann sie nicht mehr steigt oder steigen kann. Auch sollten weitere Untersuchungen näher auf die Fehlklassifizierungen eingegangen. Warum wurden diese falsch klassifiziert? Ist es vielleicht gar nicht möglich diese zu unterscheiden und sollten deshalb aus dem Datenset entfernt werden?}
		\en{It could also be shown that the accuracy of the model could be permanently improved with an increasing number of data elements (see chapter ``\nameref{sec:section_validation_number_of_train_files}''). It seems that the maximum model accuracy with 11,913 training images has not yet been reached at the end (see also chapter ``\nameref{sec:section_data_augmention}''). It should be further investigated if further increasing data sets will improve the model accuracy and from which point the model accuracy does not or can not increase any more. Further investigations should also examine the misclassifications in more detail. Why were these images classified incorrectly? Is it perhaps not possible to differentiate these images? And should they therefore be removed from the data set?}
		
		\de{In dieser Arbeit wurden die Modelle mit den Standard-Einstellungen aus dem Kapitel ``\nameref{sec:section_default_setup}'' trainiert. Es konnte gezeigt werden, dass viele der darin genannten Hyperparameter und Trainingseigenschaften als Basis für das erste Modell dienen können, um gute Modellgenauigkeiten zu erreichen: Die Anzahl der Lernepochen, die Aktivierungsfunktion, das Training aller CNN Layer, das Verwenden des TL Ansatzes und die Verwendung des kompletten Datensatz. Es gibt aber auch Ausnahmen, bei denen unter Umständen Verbesserungen erfolgen können, wenn man sie durch Vergleichstrainings ausprobiert. Es sei jedoch erwähnt, dass bei diesen Parametern keine großen Sprünge im Vergleich zum Standard-Setup zu erwarten sind. Als Beispiel genannt werden kann hier das Momentum, der Wert zum Verringern der Lernrate während der Epochen und die Batch Size. Ein interessanter Artikel zum Hyperparameter Batch Size aus dem Jahre 2018 untersucht einen interessanten Ansatz, den man ebenfalls in zukünftige Untersuchungen mit einbeziehen könnte: ``Don’t decay the learning rate, increase the batch size''\autocite{smith2017don}.}
		\en{In this thesis the models were trained with the default settings from the chapter ``\nameref{sec:section_default_setup}''. It could be shown that many of the hyper parameters and training properties mentioned there can be used as a basic for the first model to achieve good model accuracy: The number of learning epochs, the activation function, the training of all CNN layers, the use of the TL approach and the use of the complete data set. There are exceptions where improvements can be achieved if they are tried out through comparative training. It should be mentioned that no big jumps are expected in these parameters compared to the standard setup. Examples are the momentum, the value for reducing the learning rate during epochs and the batch size. An interesting article about the hyperparameter batch size from 2018 investigates an interesting approach that could also be included in future investigations: ``Don't decay the learning rate, increase the batch size''\autocite{smith2017don}.}

		\de{Desweiteren wurden in dieser Arbeit Optimierungstechniken untersucht. Einige funktionierten in gewissen Grenzen gut. Andere Ideen klangen auf den ersten Blick vielversprechend, waren es jedoch am Ende nicht. Zu den Techniken, die ein Verbesserungspotential aufweisen gehört auf jeden Fall die Data Augmentation (siehe Kapitel ``\nameref{sec:section_data_augmention}''). Es lohnt sich hier mehr Rechenzeit zu investieren, um das Modell nochmals etwas zu verbessern. Ob hier auch das Ausbalancieren des Datensatzes eine Rolle spielt, sollte weiterhin untersucht werden. Es konnte nicht ganz geklärt werden. Auch in welcher Größenordnung nicht ausbalancierte Datensätze die Modellgenauigkeit beeinflussen ist ein Punkte, den man in zukünftige Untersuchungen mit einplanen kann. Aktuelle CNNs sind gut darin Klassifkationen wie zum Beispiel von Bildern durchzuführen. Eine Einführung von hierarchischen Ebenen oder binären Klassifkationen ist hier nicht notwendig, auch wenn sie auf den ersten Blick logisch klingt und das Datensets, wie das hier verwendete \texttt{food-50} nur aus sehr wenigen Klassen besteht. Auch wurde gezeigt, dass es genügt ein einfaches DL mit der Größe der Anzahl von zu klassifizierenden Klassen am Ende des CNNs hinzuzufügen. Eine Dropout Schicht ist nicht notwendig.}
		\en{Additionally, optimization techniques were investigated in this thesis. Some of them worked well within some limits.  Other ideas seemed promising at first sight, but in the end they were not.  One of the techniques with potential for improvement is data augmentation (see chapter ``\nameref{sec:section_data_augmention}'').  Investing more computing time is recommended here to improve the model accuracy a bit more.  If the balanced data set has also influenced the model accuracy, should be investigated further. It could not be clarified completely.  Also in which dimension unbalanced data sets influence the accuracy of the model is a point that can be included in future investigations. Current \acp{cnn} are good in performing classifications such as images.  An implementation of hierarchical levels or binary classifications is not necessary here, even if it sounds logical at first sight and the data set, like the data set \texttt{food-50} used here, consists of only a few classes. It was also shown that it is sufficient to add a simple \ac{dl} with the size of the number of classes to be classified at the end of the \ac{cnn}. A dropout layer is not necessary.}

		\de{Bildklassifikationen sind ein brandaktuelles Thema in der Wirtschaft und Wissenschaft. Der Wunsch nach immer höheren Modellgenauigkeiten ist hoch, sodass auch hier viele Ideen und Verbesserungsansätze auftauchen. Wie wäre es zum Beispiel das hier genannte Datenset mit weiteren Daten aus weiteren Quellen anzureichern? Das ganze zusammen mit Data Augmentation?}
		\en{Image classifications are a hot topic in business and science. The desire for increasingly higher model accuracies is huge, so many ideas and approaches for improvements are arising here as well. For instance, how about enriching the data set mentioned here with additional data sets from other sources, like Flickr or Google? All this together with data augmentation?}

		\de{Und zu guter Letzt stellt sich noch die Frage: ``Kann man auf die großen Dienste wie Google und Microsoft verzichten?'' Um diese Frage ganz genau beantworten zu können, müssen weitere Untersuchungen vorgenommen werden. Am besten vergleicht man hierzu den gleichen Validierungsdatensatz an, wie in dieser Arbeit verwendet. Die Frage die bei dieser Untersuchung geklärt werden muss: ``Verbessern sich die Modellgenauigkeiten mit diesen öffentlichen Diensten?'' Gerade wenn es um das Thema begrenzte Domänen geht, wie z.B. Essenserkennungen im Detail, so stellt sich die Frage, ob man nicht doch besser ein eigenes angepasstes Modell erstellt. Mit Erkennungsgenauigkeiten von 85,60\% kann es durchaus in Betracht gezogen werden, eigene Modelle zu erstellen, die auf die jeweilige Problemstellung angepasst sind. Zu erwähnen seien z.B. Produktklassifizierungen, die hier genannte Essensklassifizierung oder zum Beispiel auch datenschutzrelevante Themen wie Gesichtserkennung. Allerdings immer unter dem einen wichtigen Gesichtspunkt: Daten sind teuer zu beschaffen. Deshalb sollte man sie mit Bedacht und Sorgfalt beschaffen. Auch wenn es sich am Ende nur um einen kleinen Datensatz handelt.}
		\en{And last but not least, the question arises: ``Can we do without the big services like Google and Microsoft?'' In order to be able to answer this question precisely, further research is needed. The best way to do this is to compare this services with the same validation data set used in this thesis. The question that needs to be clarified in this investigation: ``Do the model accuracies improve with these public services?'' Especially when it comes to the topic of limited domains, such as food recognition in detail, the question arises whether it is not better to create an own adapted model. With recognition accuracies of 85.60\%, it can be considered to create own models that are adapted to the specific problem. For example, product classifications, the food classification mentioned here, or for example data protection relevant topics such as face recognition should be mentioned here. But always under the one important aspect: Data is an expensive undertaking and should be obtained with caution and care. Even if in the end it is only a small data set.}
	...
	\section*{List of acronyms}
		...
		\begin{acronym}[General]
			...
			\acro{fcl}[FCL]{fully connected layer}
	...
	\begin{comment}
		...
		\section*{List of interesting links}
			...
			\begin{itemize}
				...
				\end{itemize}
				\item Stop Feeding Garbage To Your Model! — The 6 biggest mistakes with data sets and how to avoid them.
				\begin{itemize}
					\setlength\itemsep{0em}
	...
	\section{Appendix}
		...
		\subsection{Performance comparison between \ac{gpu} and \ac{cpu}}
			...
			\end{table}
			\footnotetext{Depending on the best result, which is marked with factor 1.00x}
...
model = Model(inputs = base_model.input, outputs = predictions)
		...
		\subsection{Model accuracy of binary classification}
			...
			\begin{table}[htb]
				...
				\captionof{table}{Overview of binary classification}\label{tbl:table_binary_classification_overview}
			\end{table}
	...
	\section*{Acknowledgement}
		...
		\de{Der schwierigste Teil für mich ist diese Danksagung. Weil es für mich keinen Unterschied macht, wer an erster oder letzter Stelle steht. Jede Person hat ihren Teil beigetragen und ist für diese Arbeit und mich gleich wichtig. Dennoch versuche ich es: An dieser Stelle möchte ich mich bei allen Personen bedanken, die zum Gelingen dieser Arbeit beigetragen haben. An erster Stelle: Prof. Dr. Karl Heinz Hoffmann, welcher mich ermutigt hat nach meinem abgebrochenem Studium vor über 15 Jahren dieses wieder aufzunehmen und den Abschluß nachzuholen. An zweiter Stelle David Urbansky, ein großartiger Fachmann auf dem Gebiet der Sentimentanalyse und Klassifizierungen von Dingen aller Art. Er war es, der mich die ganze Zeit Haut an Haut begleitet hat und mir mit hilfreichen Tipps und Ideen zur Seite stand. An dritter Stelle Prof. Dr. Angela Thränhardt Professorin für "Theoretische Physik - Simulation neuer Materialien" an der Technischen Universität Chemnitz, welche es mir ermöglichte ein etwas themenfremdes Gebiet in dieser Arbeit angehen zu können. An vierter Stelle meiner Familie, welche mir permanent zur Seite stand und mich bei diesem Thema unterstützt hat. Und zu guter Letzt: Danke ich allen Menschen, welche mir mit Korrekturen und Hinweisen zu dieser Arbeit zur Seite standen (auch wenn sie schon erwähnt wurden). Zu erwähnen seien die nachfolgend genannten Personen.

David Urbansky von Semknox. Mit seiner Erfahrung überhaupt auf diesem Gebiet. Viele seiner Ideen beruhen auf der Tatsache, dass Paper einen anspruchsvollen und interssanten Inhalt liefern müssen. Viele Ideen und Inhaltsvorschläge wären ohne ihn nicht möglich gewesen.

Christoph Sohrmann vom Fraunhofer Institut. Mit seiner Erfahrungen Paper zu schreiben. Viele seiner Ideen beruhen auf der Tatsache, dass Paper gewisse Erwartungen zu erfüllen haben, damit sie in der Wissenschaft Ansehen finden.

Isabel Benkert von ressourcenmangel, mit ihrer Erfahrung in englischen Texten und einem Auge für die Form und das Aussehen der Arbeit. Ihr verdanke ich gerade die Kleinigkeiten, die ich selbst niemals gesehen hätte. Englische Texte unterscheiden sich sehr halt stark von deutschen. ;) 

Nadine Sander von ressourcenmangel, mit ihrer Erfahrung für Pragmatismus und einem Auge für Dinge, die nur Sinn machen, wenn sie einen Zweck erfüllen. Ihr verdanke ich die Hinweise wie der englische Text und die Arbeit nach außen wirkt.

Angela Thränhardt mit dem wissenschaftlichen Anspruch und mehr als 100 geschriebenen englischen Veröffentlichungen. Ihre Anmerkungen zu überarbeiten war für mich der schwierigste Teil. Ihr verdanke ich die Ausdrucksweise und den Anspruch Dinge wissenschaftlich zu beschreiben. Sätze sind erst dann Sätze, wenn sie aus Subjekt und Prädikat bestehen. ;)
}

		\en{The hardest part for me is this acknowledgment. Because it doesn't make any difference to me who comes first or last. Each person has done his or her part and is equally important for this thesis and me. Nevertheless I try: At this point I would like to thank all the people who have helped me to make this thesis a success. First and foremost: \emph{Prof. Dr. Karl Heinz Hoffmann}, who encouraged me to resume my studies after I had broken off my studies more than 15 years ago and to catch up on my degree. In second place \emph{Dr. David Urbansky} from the semknox company, a great expert in the field of sentiment analysis and classifications of all kinds of things. He was the one who accompanied me skin to skin the whole time and supported me with helpful tips and ideas. In third place \emph{Prof. Dr. Angela Thränhardt} Professor for "Theoretical Physics - Simulation of New Materials" at the Chemnitz University of Technology, which enabled me to tackle a somewhat unrelated area in this work. In fourth place was my \emph{family}, who was permanently at my side and supported me with this topic. And last but not least: Thanks to all the people who helped me with corrections and hints for this thesis (even if they were already mentioned). I would like to mention the following persons:}

		\en{\emph{Dr. David Urbansky} from semknox. With his experience in this field in general. A lot of his ideas are based on the fact that papers have to deliver a demanding and interesting content. Many ideas and content suggestions would not have been possible without him.}

		\en{\emph{Dr. Christoph Sohrmann} from the Fraunhofer Institute. With his experience in writing papers. Many of his ideas are based on the fact that paper has to meet certain expectations in order to be respected in science.}

		\en{\emph{Isabel Benkert} from ressourcenmangel. With her experience in english texts and an eye for the form and appearance of this thesis. I thank her for the little things that I would never have seen myself. English texts are very different from german texts. ;)}

		\en{\emph{Nadine Sander} from ressourcemangel, with her experience for pragmatism and an eye for things that only make sense if they serve a purpose. I thank her for all english corrections and all hints about the topic ``How thesis are seen by others.''}

		\en{\emph{Prof. Dr. Angela Thränhardt} with the scientific claim and more than 100 written english publications. Revising her comments was the most difficult part for me. I thank her for the way of expression and the claim to describe things scientifically. Sentences are only sentences if they consist of subject and predicate ;)}
