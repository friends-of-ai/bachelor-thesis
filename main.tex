			...
			\label{sec:section_validation_hierarchical classification}
				...
				\en{In order to find out whether a pre-classification improves the result, the classes are divided into groups. The idea is that no longer one model predicts all classes, but rather the images are pre-sorted into a group. The corresponding group then carries out the actual classification. For this purpose, a principal component analysis is performed to analyze the similarity of the classes from the \texttt{food-50} training data set. The result will then be used to create groups. In preparation, a model will again be trained for all classes: \texttt{InceptionV3}, 21 epochs, batch size 16, learning rate \(\eta = 0.001\) decreasing by factor 0.5 every seven epochs. Using this model, a prediction is made of all training and validation data and the probability vectors are determined:}
					...
					\paragraph{\(k\)-means clustering}
						\de{Das Ergebnis aller 50 Vektoren \(\overline{\boldsymbol\lambda}_{class_{m}}\) der Gleichung \eqref{eq:equation_hierarchical_classification_mean} werden nun mittels \(k\)-means clustering untersucht. Dabei werden sechs Gruppen geclustert (\(k=8\)). Die Verteilung ist wie nachfolgend dargestellt:}
						\en{The result of all 50 vectors \(\overline{\boldsymbol\lambda}\) of the equation \eqref{eq:equation_hierarchical_classification_mean} are now examined by \(k\)-means clustering. Six groups are clustered (\(k=8\)). The distribution is as shown in Table \ref{tbl:table_grouped_classes_short_view} (a more detailed version can be found in Table \ref{fig:analysis_grouped_classes_with_k_means} in the appendix).}
							...
							{\def\arraystretch{1}\tabcolsep=5pt
								{\setlength{\extrarowheight}{2pt}
									\begin{tabular}{ l | r | r | r | r | r  }
										...
										\thead[l]{Group} & \thead[l]{\(\#_{classes}\)} & \thead[l]{\(\#_{files_{val}}\)} & \thead[l]{\(ratio_{files_{val}}\)} & \thead[l]{\(acc\)\textsuperscript{*}} & \thead[l]{\(\widehat{acc}\)\textsuperscript{*}} \\
										\hline
									preclassification & 8 & 2,953 & 100.0\% & \pbox[t]{20cm}{\flushRight 88.93\% \\ {\scriptsize @ ep. 14}} & \\
									
										\hline
										\texttt{group\textsubscript{0}} & 22 & 1,267 & 42.91\% & \pbox[t]{20cm}{\flushRight 84.17\% \\ {\scriptsize @ ep. 18}} & 74.85\% \\
				
										\texttt{group\textsubscript{1}} & 5 & 296 & 10.02\% & \pbox[t]{20cm}{\flushRight 93.21\% \\ {\scriptsize @ ep. 16 }} & 82.89\% \\
				
										\texttt{group\textsubscript{2}} & 4 & 249 & 8.43\% & \pbox[t]{20cm}{\flushRight 94.85\% \\ {\scriptsize @ ep. 11 }} & 84.35\% \\
				
										\texttt{group\textsubscript{3}} & 1 & 63 & 2.13\% & 100.0\% & 88.93\% \\
				
										\texttt{group\textsubscript{4}} & 2 & 158 & 5.35\% & \pbox[t]{20cm}{\flushRight 99.30\% \\ {\scriptsize @ ep. 10 }} & 88.31\% \\
				
										\texttt{group\textsubscript{5}} & 12 & 677 & 22.93\% & \pbox[t]{20cm}{\flushRight 85.63\% \\ {\scriptsize @ ep. 11 }} & 76.15\% \\
				
										\texttt{group\textsubscript{6}} & 3 & 160 & 5.42\% & \pbox[t]{20cm}{\flushRight 93.13\% \\ {\scriptsize @ ep. 5 }} & 82.82\% \\
