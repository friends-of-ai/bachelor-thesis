...
\begin{document}
	...
	\section{Background}
		...
		\label{sec:section_training_test_and_evaluation}
			...
			\de{Eine optimale Aufteilung des Trainings- und Validierungsdatensatzes ist abhängig von dem vorliegenden Klassifizierungsproblem und die Anzahl der Daten, welche zur Verfügung stehen. In dieser Arbeit wird nach dem Pareto Prinzip ein Verhältnis aus 80 Prozent Trainingsdaten und 20 Prozent Validierungsdaten verwendet, sofern nicht anders angegeben.}
			\en{An optimal division of the training and validation data set depends on the existing classification problem and the amount of data available. In this thesis a ratio of 80 percent training data and 20 percent validation data is used according to the Pareto principle, unless otherwise stated.}
	...
	\subsection{Classification metrics and confusion matrix}
		...
		\subsubsection{Confusion matrix}
			...
			\end{table}
		 	...
		 	\de{Zum Schluß besitzt die Confusion Matrix folgenden Aufbau, wobei die Anzahl von Elementen in der Klasse \(P_{i}\) vorhergesagt wurde, obwohl (\(\cong\)) es hätte Klasse \(A_{j}\) sein müssen. Alle Werte auf der Diagonalen wurden richtig vorhergesehen.}
			\en{Finally, the confusion matrix has the following structure, where the number of elements in the class \(P_{i}\) was predicted (P - predicted) although (\(\cong\)) it should have been class \(A_{j}\) (A - actual). All values on the diagonal were correctly predicted (\(\widehat{=}\), \(i\) and \(j\) are equal).}

			\begin{equation}
				\boldsymbol{M}_{confusion} = \begin{bmatrix}
					\#(P_{1}\widehat{=} A_{1}) & \dots & \#(P_{n}\cong A_{1}) \\
					\vdots & \ddots & \vdots \\
					\#(P_{1}\cong A_{n}) & \dots & \#(P_{n}\widehat{=} A_{n})
				\end{bmatrix} = (m_{nn})
			\end{equation}

			\de{Wenn alle Elememte neben der Hauptdiagonalen 0 sind, so wurden alle Klassen richtig vorhergesehen.}
			\en{If all elements next to the main diagonal are 0, then all classes were correctly predicted.}

			\begin{equation}
				\boldsymbol{M}_{confusion} = \begin{bmatrix}
					\#(P_{1}\widehat{=} A_{1}) & \dots & 0 \\
					\vdots & \ddots & \vdots \\
					0 & \dots & \#(P_{n}\widehat{=} A_{n})
				\end{bmatrix} = (m_{nn})
			\end{equation}
